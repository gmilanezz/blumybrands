...
const STORAGE_KEY = 'blummybrands_products';
...
export class ProductService {
  ...
  private loadInitialProducts(): Product[] {
    ...
    try {
      ...
      const basicImagesByCode = new Map(
        MOCK_PRODUCTS
          .filter(product =>
            this.normalize(product.brand) === 'esmeral' &&
            this.normalize(product.catalog) === 'basic'
          )
          .map(product => [product.code, product.images] as const)
      );

      return parsedProducts.map(product => {
        const basicImages = basicImagesByCode.get(product.code);
        return basicImages ? { ...product, images: [...basicImages] } : product;
      });
    } catch {
      return [...MOCK_PRODUCTS];
    }
